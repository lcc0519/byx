...
//					console.log(i);
					...
					if (user.head_pic == "") {
						...
						header_img.src = BASE_URL + user.head_pic.substring(3);
//						console.log(BASE_URL + user.head_pic.substring(3));
					}
...
//							li_content_grid_cell.style.height = '200px';
							...
							li_content_grid_cell.innerHTML = '<a href="#">\
											<img src="'+ pic_path +'" width="100%" data-preview-src="" data-preview-group="'+ i + '" />\
									        </a>';
						...
						for (var j = 0; j < pic_num; j++) {
							var pic_path = BASE_URL+info.tiezi[i].pic[j].path.substr(3);
							var li_content_grid_cell = document.createElement("li");	
//							li_content_grid_cell.style.height = '100px';
							li_content_grid_cell.className = 'mui-table-view-cell mui-media mui-col-xs-6 mui-col-sm-6';
							li_content_grid_cell.innerHTML = '<a href="#">\
											<img src="'+ pic_path +'" width="100%" height="90px" data-preview-src="" data-preview-group="'+ i + '" />\
									        </a>';		
							ul_content_grid.appendChild(li_content_grid_cell);		        
						}
					}else if(pic_num >= 3){
...
//							li_content_grid_cell.style.height = '80px';
							li_content_grid_cell.innerHTML = '<a href="#">\
												<img src="'+ pic_path +'" width="100%" height="90px" data-preview-src="" data-preview-group="'+ i + '" />\
										        </a>';
							ul_content_grid.appendChild(li_content_grid_cell);		
...
//							if(info.tiezi[i].like[0].created_username == user.nickname){
//								div_praise.my=1;
//								div_praise.total=1;
//							}
...
//								if(info.tiezi[i].like[n].created_username == user.nickname){
//									div_praise.my=1;
//									div_praise.total=1;
//								}
//								console.log("##like------"+ n);
...
//			console.log('boxid:'+id);
			...
			if (p_style.display=='inline-block'||m_style.display=='inline-block') {
				console.log("^^^^^^showcommitbox11111");
				like_time = 1;
				obj_id = setTimeout(function () {
					console.log("^^^^^^showcommitbox22222");
...
//				console.log('*****value.length:' +praises.innerHTML.length);
				praises.innerHTML = praises.innerHTML +user.nickname;
			}else if(praises.innerHTML.indexOf(', '+user.nickname ) > 0){
//				console.log("您已点赞11111！");
//				console.log('*****value.length:' +like_value.length);
				like_value=like_value.replace(', '+user.nickname,"");
				praises.innerHTML = like_value;
//				console.log('*****1111value.length:' +praises.innerHTML.length);
			}else if(praises.innerHTML.indexOf(user.nickname ) > 0){
//				console.log("您已点赞22222！");
//				console.log('*****value.length:' +like_value.length);
				if(praises.innerHTML.indexOf(user.nickname+ ', ' ) > 0){
					like_value=like_value.replace(user.nickname+', ',"");
				}else{
					like_value='';
				}				
...
//				console.log(el.getAttribute("rpid"));
	            switch (el.title) {
					...
					case 'praise':
						...
						sendReply (el.parentNode.parentNode,el,user.nickname, user.id); //apple add
//						praiseBox(el.parentNode.parentNode,el);
						break;
					//点赞
					case 'msg':  //添加新贴
...
//						text += "拍照或录像";
						...
						var parent_node = el.parentNode.parentNode.parentNode.parentNode;
						var childe_node = el.parentNode.parentNode.parentNode;
